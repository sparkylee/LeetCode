class Solution {
     int width,height;
        ...
        private boolean isPositivePoint(char[][] matrix,int i,int j)
        {
            return (i>=0 && i< this.width && j>=0 && j<this.height && matrix[j][i]!='0');
        }
        private int maximalSquare(char[][] matrix,int start_i,int start_j,int end_i,int end_j) {
            while (true)
            {
                ...
                if(TE && LE && isPositivePoint(matrix,start_i-1,start_j-1))
                {
                    start_i = start_i - 1;
                    start_j = start_j - 1;
                    continue;
                }
                if(TE && RE && isPositivePoint(matrix,end_i+1,start_j-1))
                {
                    end_i = end_i + 1;
                    start_j = start_j - 1;
                    continue;
                }
                if(RE && BE && isPositivePoint(matrix,end_i+1,end_j+1))
                {
                    end_i = end_i + 1;
                    end_j = end_j + 1;
                    continue;
                }
                if(BE && LE && isPositivePoint(matrix,start_i-1,end_j+1))
                {
                    start_i = start_i - 1;
                    end_j = end_j + 1;
                    continue;
        ...
        public int maximalSquare(char[][] matrix) {
            ...
            while(true)
            {
                ...
                int area_new = maximalSquare(matrix, start_i, start_j, start_i, start_j);
                this.area = Math.max(area_new,this.area);
            }
            return this.area;
        }
