class Solution {
    ...
    vector<set<char>> hset;
    vector<set<char>> vset;
    vector<set<char>> gset;
    ///////////////////////////////
    ...
    */
    ...
    ////////////////////////////
    void printSingleVectorSet(vector<set<char>> sv)
    {
    ...
    void initSet(vector<vector<char>> & board)
    {
        ...
        for(int i = 0; i < 9; i ++)
        {
            ...
            for(int j = 0; j < 9; j ++)
            {
                hset[i].insert(board[i][j]);
                vset[j].insert(board[i][j]);
                gset[gIndex(i,j)].insert(board[i][j]);
            }
        }
    ...
    {
        return (gset[gIndex(i,j)].count(c) == 0) && (hset[i].count(c) == 0) && (vset[j].count(c)==0);
    }

    void insertChar(vector<vector<char>> & board,int i, int j, char c)
    {
        board[i][j] = c;
        gset[gIndex(i,j)].insert(c);
        hset[i].insert(c);
        vset[j].insert(c);
    }
    void eraseChar(vector<vector<char>> & board,int i, int j, char c)
